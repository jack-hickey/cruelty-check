document.querySelectorAll(".btn--feedback").forEach(e=>Object.assign(e,{textContent:Localizer.FEEDBACK_BUTTON_LABEL,onclick:()=>{Dialog.ShowCustom(Localizer.FEEDBACK_TITLE,Localizer.FEEDBACK_DESC,`
			<chip-form>
				<chip-dropdown
		...
		`,{Size:"md",Scrollable:!0,OnCheckValid:e=>e.querySelector("chip-form").reportValidity(),AffirmativeText:"Submit"}).then(()=>report(drpType.value,"User Submitted Feedback",txtDetails.value))}}));let resultsContainer=document.getElementById("ctResults");function resetMobileView(){txtSearch.blur()}function search(){let t=txtSearch.value.trim();t&&(resultsContainer.innerHTML='<chip-loading class="d-flex m-auto mt-form--lg" size="xl"></chip-loading>',txtSearch.blur(),Product.search(t).then(e=>{resultsContainer.innerHTML="",txtResultCount.innerHTML="",txtResultCaption.innerHTML="",e.length?displayResults(e):(resultsContainer.appendChild(document.createElementWithContents("chip-emptyprompt",Localizer.EMPTY_SEARCH_DESC,{heading:Localizer.EMPTY_SEARCH_TITLE,icon:"far fa-cat",id:"epResults",className:"mt-form--lg"})),lblSearchTerm.textContent=t),document.querySelectorAll(".btn--add-product").forEach(e=>e.onclick=()=>Product.add())}))}function displayResults(e){resultsContainer.addItems(e.map(e=>buildResult(e))),txtResultCount.innerHTML=1!==e.length?Localizer.SEARCH_RESULTS_TITLE.replace("{count}",`<span class="fw-bold">${e.length}</span>`):Localizer.SEARCH_RESULT_TITLE,txtResultCaption.innerHTML=Localizer.SEARCH_RESULT_CAPTION}function report(e,t,i,a){Ajax.Post("report",{body:{type:e,title:t,description:i},success:{ok:e=>{a||Dialog.ShowSuccess(Localizer.FEEDBACK_SUCCESS_TITLE,Localizer.FEEDBACK_SUCCESS_DESC)}}})}function buildResult(e){var t=e.getAdvisories(),t=document.createElementWithContents("chip-card",`
		...
		`,{image:"/cdn-cgi/image/width=200,quality=80,format=auto/products/"+e.Image,hideBlur:!0});return t.classList.add("cd--product"),t.querySelector(".btn--report-product").onclick=()=>e.reportIncorrect(),t}Object.assign(window.txtSearch??{},{onsuffixclick:()=>search(),onkeyup:e=>{"Enter"===e.key&&search()}});